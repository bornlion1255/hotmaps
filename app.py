...
def ask_ai(system_prompt, user_content):
    ...
    payload = {
        "model": "openai/gpt-4o", # Самая мощная модель для анализа
        "messages": [
    ...
    try:
        r = requests.post("https://routerai.ru/api/v1/chat/completions", json=payload, headers=headers)
        return r.json()['choices'][0]['message']['content']
    except Exception as e:
        return f"AI сейчас занят анализом (Ошибка: {e})"
...
if check_password():
    ...
    if btn:
        ...
        if not df.empty:
            ...
            full_context = df[['date', 'rating', 'text', 'location_address', 'emotions']].to_string()
