...
def get_coords(address):
    try:
        # Пауза срабатывает ТОЛЬКО если адреса нет в памяти и мы реально стучимся в API
        time.sleep(0.5) 
...
# 3. Синхронизируем данные и СОЗДАЕМ df
    with st.spinner("Синхронизация с базой Hotmaps..."):
        data = fetch_data(start, end)
        df = pd.DataFrame(data)
        
        if not df.empty:
            # 1. Форматируем даты и убираем временные зоны для точного сравнения
            df['date'] = pd.to_datetime(df['date'], utc=True).dt.tz_localize(None)
            
            # 2. Оставляем только Химчистку
            df = df[(df['location_group'] == 'химчистка') | ((df['location_group'] == 'otzovik') & (df['store_code'] == 'otzovik_dry_cleaning'))]
            
            # 3. ЖЕСТКАЯ ФИЛЬТРАЦИЯ ПО ДАТАМ (Защита от сбоев API Hotmaps)
            start_dt = pd.to_datetime(start)
            end_dt = pd.to_datetime(end) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1) # Берем до 23:59:59 последнего дня
    ...
    with tab1:
        ...
        with col_chart1:
            ...
            if 'emotions' in df.columns:
                # Добавили прозрачный фон и увеличили внутренний радиус для стиля
                fig_pie = px.pie(df, names='emotions', hole=0.55, 
                                 color='emotions', 
                                 color_discrete_map={'POSITIVE':'#2ecc71', 'NEGATIVE':'#e74c3c', 'NEUTRAL':'#95a5a6'})
            else:
                fig_pie = px.pie(df, names='rating', hole=0.55)
...
with col_chart2:
            # Считаем, сколько дней выбрал пользователь
            days_selected = (end - start).days
            
            # Динамическое переключение: до 14 дней -> по дням, больше -> по неделям
            if days_selected <= 14:
                period_title = "дням"
                df['time_period'] = df['date'].dt.to_period('D').apply(lambda r: r.start_time)
            else:
                period_title = "неделям"
                df['time_period'] = df['date'].dt.to_period('W').apply(lambda r: r.start_time)
                
            st.subheader(f"Динамика качества (По {period_title})")
            
            # Агрегация по выбранному периоду
            trend = df.groupby('time_period').agg(mean_rating=('rating', 'mean'), count_reviews=('id', 'count')).reset_index()

            fig_trend = go.Figure()
            
            # 1. Фоновый объем (Area Chart)
            fig_trend.add_trace(go.Scatter(
                x=trend['time_period'], y=trend['count_reviews'],
                name='Кол-во отзывов',
                fill='tozeroy',       
                mode='none',          
            ...
            # 2. Плавная линия тренда (Spline)
            fig_trend.add_trace(go.Scatter(
                x=trend['time_period'], y=trend['mean_rating'],
                mode='lines+markers',
                name='Средний рейтинг',
                line=dict(color='#2ecc71', width=4, shape='spline'), 
                marker=dict(size=8, color='#2ecc71', line=dict(width=2, color='#222')) 
            ))

            # Настройка осей и прозрачности
            fig_trend.update_layout(
                yaxis=dict(title="Рейтинг", range=[1, 5.2], gridcolor='rgba(255,255,255,0.05)', zeroline=False),
                yaxis2=dict(title="Объем (шт)", overlaying='y', side='right', showgrid=False, zeroline=False),
            ...
            # 2. Плавная линия тренда (Spline)
            fig_trend.add_trace(go.Scatter(
                x=trend['week'], y=trend['mean_rating'],
                mode='lines+markers',
                name='Средний рейтинг',
                line=dict(color='#2ecc71', width=4, shape='spline'), # Сглаживание углов
                marker=dict(size=8, color='#2ecc71', line=dict(width=2, color='#222')) # Красивые точки
            ))
    ...
    with tab3:
        ...
        map_data['avg_rating'] = map_data['avg_rating'].round(1)
        ...
        if not map_data.empty:
            st.pydeck_chart(pdk.Deck(
                ...
                tooltip={"html": "<b>{location_address}</b><br/>Средний рейтинг: <b>{avg_rating}</b><br/>Кол-во отзывов: <b>{count_rev}</b>"}
            ))
        else:
