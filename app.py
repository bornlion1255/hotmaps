...
# 3. Синхронизируем данные и СОЗДАЕМ df
    ...
    with tab1:
        ...
        with col_chart2:
            st.subheader("Динамика качества (По неделям)")
            
            # Агрегация по неделям для чистоты данных
            df['week'] = df['date'].dt.to_period('W').apply(lambda r: r.start_time)
            trend = df.groupby('week').agg(mean_rating=('rating', 'mean'), count_reviews=('id', 'count')).reset_index()

            fig_trend = go.Figure()
            
            # 1. Фоновый объем (Area Chart вместо забора из столбиков)
            fig_trend.add_trace(go.Scatter(
                x=trend['week'], y=trend['count_reviews'],
                name='Кол-во отзывов',
                fill='tozeroy',       # Заливка до низа
                mode='none',          # Без жесткой верхней линии
                fillcolor='rgba(52, 152, 219, 0.15)', # Мягкий синий цвет
                yaxis='y2'
            ))
